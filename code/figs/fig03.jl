    ...
    for col in 1:n_col
        ...
        if (col == 1) & (row == 1)
            ...
            for (rep, (key, value)) in enumerate(sort(rep_vars))
                ...
                # Add title
                ax[rep].title = "replicate $key | neutrals"
                ax[rep].titlesize = 12
            end # for
        else
            # Loop through replicates
