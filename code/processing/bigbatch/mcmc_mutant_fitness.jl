...
df = df[
    (df.batch.==batch).&(df.hub.==hub).&(df.perturbation.==perturbation),
    :]
...
for (i, data) in enumerate(df_group)
    ...
    if (rm_T0) & (isfile("$(out_dir)/$(fname)_mcmcchains_rmT0.jld2"))
        println("$fname was already processed")
        continue
    elseif (!rm_T0) & (isfile("$(out_dir)/$(fname)_mcmcchains.jld2"))
        println("$fname was already processed")
        continue
    end # if
    ...
    if rm_T0
        files = mean_files[
            occursin.(batch, mean_files).&occursin.(hub, mean_files).&occursin.("$(perturbation)perturbation", mean_files).&occursin.(rep, mean_files).&.!(occursin.("T0", mean_files))
        ]
    else
        files = mean_files[
    ...
    if rm_T0
        JLD2.jldsave(
            "$(out_dir)/$(fname)_mcmcchains_rmT0.jld2",
