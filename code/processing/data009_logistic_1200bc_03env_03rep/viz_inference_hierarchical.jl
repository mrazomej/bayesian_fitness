...
# Define file
file = first(Glob.glob("./output/advi_meanfield_01samples*"))
...
df_advi = BayesFitness.utils.advi2df(
    dist, vars, mut_ids; n_rep=3, envs=[1, 1, 2, 3, 1, 2, 3]
)
...
for (i, p) in enumerate(rep_pairs)
    ...
    # Plot identity line
    lines!(ax[i], repeat([[-2, 1.5]], 2)..., linestyle=:dash, color="black")
...
for rep in 1:n_rep
    # Plot identity line
    lines!(
        ax[rep], repeat([[-2, 1.5]], 2)..., linestyle=:dash, color="black"
    )
...
# Extract information
data_advi = df_advi[
    df_advi.vartype.=="mut_hyperfitness", [:id, :mean, :std, :env]
]
...
for env = 1:n_env
    ...
    scatter!(
        ax,
        data[(data.env.==env), :fitness],
        data[(data.env.==env), :mean],
        markersize=8,
        color=(env_colors[env], 0.2)
    )
end # for
...
for env = 1:n_env
    ...
    ecdfplot!(
        ax,
        abs.(data[data.env.==env, :mean] .- data[data.env.==env, :fitness]),
        label="env$env",
...
for env = 1:n_env
    # Compute Z-score of true fitness values
    fitness_zscore = (
        data[data.env.==env, :mean] .- data[data.env.==env, :fitness]
    ) ./ data[data.env.==env, :std]
    # Plot ECDF
    ecdfplot!(
        ax,
        abs.(fitness_zscore),
