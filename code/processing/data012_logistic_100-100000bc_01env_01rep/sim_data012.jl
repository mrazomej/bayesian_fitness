...
# Loop through list of barcodes to generate
for (n_neutral, n_mut) in n_bcs
    ...
    if gen_plots
        ...
        save("./output/figs/trajectories_$(lpad(n_bc,6,"0")).pdf", fig)
    end # if
    ...
    out_dir = "$(git_root())/data/logistic_growth/data_012"
    ...
    CSV.write("$(out_dir)/tidy_data_$(lpad(n_bc,6,"0")).csv", data)
end # for