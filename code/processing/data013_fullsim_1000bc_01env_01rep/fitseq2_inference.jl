...
# Convert data to CSV for FitSeq2 -t input
bc_cell_mat = DF.combine(DF.groupby(data, :time), :count_noiseless => sum)

# Transform first column to the number of generations in experiment
bc_cell_mat[:, :time] = (bc_cell_mat[:, :time] .* n_gen) .- n_gen
...
CSV.write(cell_path, bc_cell_mat, writeheader=false)
