...
ax = Axis(
    gl_comp[1, 1],
    xlabel="ground truth Malthusian fitness",
...
lines!(
    ...
    repeat(
        [
            [
                minimum(df_fitness.fitseq2_fitness),
                maximum(df_fitness.fitseq2_fitness)
            ] .* 1.1
        ],
        2
    )...;
...
# Plot comparison
scatter!(
    ax, df_fitness.fitness, df_fitness.fitseq2_fitness, markersize=8
)
...
save("$(git_root())/doc/figs/figSIX_fitseq2_sim.pdf", fig)
save("$(git_root())/doc/figs/figSIX_fitseq2_sim.png", fig)
