...
of all the dimensions must add up to one. A natural choice for a probability
...
$${#eq-freq_likelihood}
which again is simply read as the number of reads at time $t$ $\underline{r}_t$
given the frequencies at the same time $\underline{f}_t$ is distributed with
a multinomial distribution parametrized by $R_t$, the total number of reads, and
...
$${#eq-freq_posterior}
where the $\underline{\alpha}_t$ parameters are updated to $\underline{\beta}_t$
