...
time $t$. Given this model limitation, we are led to assume that we must need
to infer each $\bar{s}_t$ independently. Expressing this for our inference
results in
$$
\pi(
    \underline{\bar{s}}_T \mid 
...
of the $N$ neutral lineages' frequencies is changing independently. Their
...
hand and fitness on the other results in
$$
\frac{f_{t+1}^{(n)}}{f_t^{(n)}} = \mathrm{e}^{- \bar{s}_t}.
...
independent frequencies to the mean fitness. Therefore, instead of writing the
joint distribution of the frequency values at times $t$ and $t+1$ conditioned on
the mean fitness for the likelihood, we write the joint distribution of the
...
$\gamma_t^{(n)}$ and $f_{t+1}^{(n)}$ for a given value of $f_t^{(n)}$.
...
where we added an extra term, $\varepsilon_t^{(n)}$, characterizing the 
deviations of the measurements from the theoretical model. It is common to 
assume that these errors are normally distributed with mean zero and some 
standard deviation $\sigma_t$, implying that
$$
...
that matter. As a matter of fact, given that we only get to observe the
frequency ratio $\gamma_t^{(n)}$, we can only restrict the values of the
frequency $f_{t}^{(n)}$ to be
$$
...
$$
\pi(\underline{\bar{s}}_T \mid \underline{\underline{F}}) \propto
\prod_{t=1}^{T-1} \left\{
    \prod_{n=1}^N \left[
