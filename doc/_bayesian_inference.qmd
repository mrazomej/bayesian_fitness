...
the denominator on the right-hand side of @eq-bayes_obj, and write
$$
...
on the right-hand side of @eq-bayes_obj must be computed via an integral of the
form
$$
\pi(\underline{\underline{R}}) = \int d\underline{s}^M
\pi(\underline{\underline{R}} \mid \underline{s}^M) 
\pi(\underline{s}^M),
$${#eq-bayes_evidence}
also known as a marginalization integral.
...
the statistical dependencies of our observations and latent variables allow us
to factorize the joint distribution into the product of multiple conditional
distributions. To exemplify this, let us focus on the variable
$\underline{\underline{F}}^M$ within the diagram in @fig-01(C). The incoming
arrows from $\underline{\bar{s}}_T$ and $\underline{s}^M$

rewrite the joint probability distribution as a product of conditional
...
uncertainty listed in @sec-fitness_model in each term. Starting from
...
work through these terms to build the posterior distribution to sample
