...
However, the frequencies $\underline{f}_t$, and $\underline{f}_{t+1}$, and the
standard deviation $\sigma_t$ represent nuisance parameters we ignore at the end
of the inference procedure. In other words, what we care about is the marginal
...
computational challenge we must overcome.


Since the
inference of the mean fitness $\bar{s}_t$ makes use only of the neutral lineages
frequencies, it is convenient to split the $\underline{f}_t$ vectors into the
neutral and mutant-only frequency arrays and write
$$
\pi(
...
\pi(
    \bar{s}_t, \sigma_t, 
    \underline{f}_t^N, \underline{f}_t^M, 
    \underline{f}_{t+1}^N, \underline{f}_{t+1}^M \mid
    \underline{r}_t, \underline{r}_{t+1}
)
$${#eq-meanfit_split_f}
