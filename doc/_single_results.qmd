...
to the neutral barcodes ($s^{(m)} > 0$). This experimental condition is required
to reliably measure the relative fitness since all barcodes of interest occupy a
...
observational noise input into our statistical model. This noise is evident for
the darker lines representing the neutral barcodes since all of these lineages
are assumed to be identically distributed under this log frequency ratio
transformation.
...
datasets in agreement with the inferred values. @fig-02(C) shows these results
for all neutral lineages (upper left corner plot) and a few representative
non-neutral barcodes. The different color shades represent the 95\%, 68\%, and
5\% credible regions, i.e., the regions where we expect to find the data with
the corresponding probability---or in terms of our parameter, the $X\%$ credible
region is the interval where we expect the true parameter to lie with $X\%$
probability. 
...
frequency trajectories. (B) Examples of the posterior predictive checks for all
...
on the mutant relative fitness marginal distribution. (C) Comparison between the
...
value of the ground truth parameter value within the inferred fitness posterior
distribution.](./figs/fig02){#fig-02}